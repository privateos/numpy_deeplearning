...
def autotrain(X, Y, w, b):
    ...
    w = torch.autograd.Variable(torch.from_numpy(w).float(), requires_grad=True)
    b = torch.autograd.Variable(torch.from_numpy(b).float(), requires_grad=True)
...
autotrain(X, Y, np_w_copy, np_b_copy)
torchtrain(X, Y)